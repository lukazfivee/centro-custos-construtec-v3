// Cobrancas (clientes/acompanhamento financeiro/e-mail) + sincronizacao geral
// de dados (obras/categorias/fornecedores/lancamentos) + feed de atividade,
// restaurado a partir de cloudflare-sync-worker/src/index.js apos o mesmo
// incidente que quebrou o login corporativo (ver centralAuth.js). Usa o
// mesmo banco D1 "centro-custos-producao" — as tabelas de Cobrancas/sync
// (clients, client_followups, client_email_drafts, client_email_events,
// sync_entities, sync_events, sync_clients) ja existem la, criadas pelo
// worker antigo; nada foi perdido.
import {
  json, text, validCorporateEmail, bytesToBase64, base64ToBytes,
  sha256Text, consumeRate, sessionUser, requireSession,
} from './centralAuth.js';
...
async function handleActivity(request, env) {
  ...
  const rows = (await env.DB.prepare(`
    SELECT e.id,e.entity_type,e.payload,e.resolution,e.created_at,e.source_instance_id,e.source_instance_name,e.source_user_email,
      (SELECT u.name FROM cloud_users u WHERE u.org_id=e.org_id AND u.email=e.source_user_email ORDER BY (u.deleted_at IS NULL) DESC,u.created_at DESC LIMIT 1) AS source_user_name
    FROM sync_events e
...
// Cuida de todo o restante de /v1/* que nao seja identidade central
// (ver centralAuth.js): Cobrancas, feed de atividade e sincronizacao geral
// de dados. Sempre devolve uma Response (nunca null) — assim nenhuma
// requisicao /v1/* cai no fallback do Container, que serviria a pagina
// HTML do app disfarcada de resposta de API (o bug que deixava a tela de
// Cobrancas parecer "sem dados" em vez de mostrar um erro real).
export async function handleCommercialSync(request, env) {
  const url = new URL(request.url);
  const ip = request.headers.get('cf-connecting-ip') || 'unknown';
  const allowed = await consumeRate(env.DB, ip, 'api', 5000);
  ...
  if (url.pathname === '/v1/sync' || url.pathname === '/v1/sync/snapshot') {
