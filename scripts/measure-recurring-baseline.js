// Baseline measurement for POST /api/recorrentes/gerar
// Usage: node scripts\measure-recurring-baseline.js
const fs = require('fs');
const os = require('os');
const path = require('path');
const crypto = require('crypto');

process.env.PGLITE_DATA_DIR = path.join(os.tmpdir(), 'centro-custos-rec-baseline');
process.env.RESTORE_ROOT_DIR = path.join(os.tmpdir(), 'centro-custos-rec-restore');
...
// server.js runs dotenv.config() which reloads .env (incl. DATABASE_URL).
// Load it first, then strip DATABASE_URL so we use PGlite for the test.
require('../server');
