...
router.get('/', asyncRoute(async (req,res) => {
  ...
  if (!wantsPagination(req.query)) {
    const { rows } = await getDb().query(`${SUPPLIERS_SELECT} ORDER BY ${orderBy} LIMIT 500`);
  ...
  const [dataResult, countResult] = await Promise.all([
    getDb().query(`${SUPPLIERS_SELECT} ORDER BY ${orderBy} LIMIT $1 OFFSET $2`, [limit, offset]),
    getDb().query('SELECT COUNT(*)::int AS total FROM suppliers'),
  ]);
