...
router.post('/conflitos/:id/resolver', asyncRoute(async (req, res) => {
  ...
  if (!['local','recebido'].includes(escolha)) throw httpError(400,'Escolha "local" ou "recebido".');

  const db = getDb();
  await db.transaction(async (tx) => {
    const { rows } = await tx.query('SELECT * FROM sync_conflicts WHERE id = $1', [id]);
    if (!rows[0]) throw httpError(404,'Conflito não encontrado.');
    const conflict = rows[0];
    if (conflict.status !== 'pending') throw httpError(400,'Este conflito já foi resolvido.');

    const data = escolha === 'local' ? conflict.local_data : conflict.incoming_data;
    if (!data) throw httpError(400,'Dados da versão escolhida não disponíveis.');

    const pubId = conflict.transaction_public_id;
    const existing = await tx.query('SELECT id FROM transactions WHERE public_id = $1', [pubId]);
    const centerMap = await tx.query('SELECT id, LOWER(code) AS key FROM cost_centers');
    const categoryMap = await tx.query('SELECT id, LOWER(name) AS key, type FROM categories');
    ...
    if (!centerId || !cat) throw httpError(400,'Centro ou categoria da versão escolhida não encontrado no cadastro local.');
    ...
    await tx.query("UPDATE sync_conflicts SET status='resolved' WHERE id=$1", [id]);
  });
