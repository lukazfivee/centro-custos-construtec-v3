...
router.get('/atencao',asyncRoute(async(req,res)=>{
  ...
  if(budget.rows[0].quantidade)items.push({tipo:'orcamento',nivel:'alto',titulo:`${budget.rows[0].quantidade} centro(s) acima do orçamento`,detalhe:`Revise custos e orçamento mensal de ${month}.`,mes:month});
  ...
  res.json({mes:month,total:items.length,itens:items});
}));

router.get('/fluxo-caixa',asyncRoute(async(req,res)=>{
  const horizons=[7,15,30,60,90];
  const {rows}=await getDb().query(`SELECT type,due_date::text AS vencimento,SUM(amount*accounting_sign) AS valor FROM ${financialTransactionsSql} t WHERE deleted_at IS NULL AND financial_status='pendente' AND due_date IS NOT NULL GROUP BY type,due_date ORDER BY due_date`);
...
router.get('/curva-abc',asyncRoute(async(req,res)=>{
  const {rows}=await getDb().query(`SELECT c.name AS categoria,SUM(t.amount*t.accounting_sign) AS total FROM ${financialTransactionsSql} t JOIN categories c ON c.id=t.category_id WHERE t.deleted_at IS NULL AND t.type='despesa' GROUP BY c.id,c.name HAVING SUM(t.amount*t.accounting_sign)>0 ORDER BY total DESC`);
...
router.get('/fornecedores-precos',asyncRoute(async(req,res)=>{
  const {rows}=await getDb().query(`SELECT counterparty AS fornecedor,COUNT(*)::int AS compras,ROUND(AVG(amount)::numeric,2) AS media,MIN(amount) AS menor,MAX(amount) AS maior,MAX(transaction_date)::text AS ultima_compra FROM ${financialTransactionsSql} t WHERE deleted_at IS NULL AND reversal_of IS NULL AND type='despesa' AND counterparty IS NOT NULL GROUP BY counterparty ORDER BY compras DESC,MAX(transaction_date) DESC LIMIT 100`);
  res.json(rows);
}));

router.get('/tendencia-obras',asyncRoute(async(req,res)=>{
  const {month,range}=reportMonth(req.query);
  const {rows}=await getDb().query(`SELECT cc.id,cc.code AS codigo,cc.name AS obra,cc.monthly_budget AS orcamento,cc.contract_amount AS contrato,COALESCE(SUM(t.amount*t.accounting_sign) FILTER(WHERE t.type='despesa' AND t.financial_status='liquidado'),0) AS realizado,COALESCE(SUM(t.amount*t.accounting_sign) FILTER(WHERE t.type='despesa' AND t.deleted_at IS NULL AND t.financial_status='pendente'),0) AS comprometido_pendente FROM cost_centers cc LEFT JOIN ${allocatedTransactionsSql} t ON t.cost_center_id=cc.id AND t.transaction_date >= $1 AND t.transaction_date < $2 WHERE cc.active=TRUE GROUP BY cc.id,cc.code,cc.name,cc.monthly_budget,cc.contract_amount ORDER BY cc.name`,[range.start,range.end]);
  res.json(rows.map(r=>{const realizado=Number(r.realizado),pendente=Number(r.comprometido_pendente),orcamento=Number(r.orcamento);return{...r,mes:month,realizado,comprometido_pendente:pendente,tendencia:realizado+pendente,saldo_orcamento:orcamento-(realizado+pendente),desvio_percentual:orcamento?((realizado+pendente-orcamento)/orcamento*100):0};}));
}));
