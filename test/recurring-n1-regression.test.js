...
process.env.PGLITE_DATA_DIR = path.join(os.tmpdir(), 'centro-custos-rec-n1');
process.env.RESTORE_ROOT_DIR = path.join(os.tmpdir(), 'centro-custos-rec-restore-n1');
...
test('POST /api/recorrentes/gerar não é N+1 (queries O(1) em relação a N)', async () => {
  const app = createApp();
  await setup();
  const db = getDb();
  const prereq = await seedPrerequisites(db);
  await seedTemplates(db, prereq, 50);
  ...
  assert.ok(loginResp.ok, `login failed: ${loginResp.status} ${JSON.stringify(loginData)}`);
  ...
  assert.equal(data.gerados, 50, `esperados 50 gerados, recebeu ${data.gerados}`);
  // Anti-N+1: queries devem ser O(1), não O(N). 50 templates não deve gerar 100+ queries.
  assert.ok(snap.database.total <= 8, `queries demais para 50 templates (N+1?): ${snap.database.total}`);
  assert.ok(ms(t0, t1) < 5000, `latência alta: ${ms(t0, t1)}ms`);

  const count = (await db.query("SELECT COUNT(*)::int AS total FROM transactions")).rows[0].total;
  assert.equal(count, 50, `transações criadas devem ser 50, recebeu ${count}`);
