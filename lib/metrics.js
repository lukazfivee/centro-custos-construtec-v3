...
function recordJob({ type = 'unknown', status, durationMs }) {
  state.jobs.total += 1;
  const duration = Number.isFinite(durationMs) ? durationMs : 0;
  state.jobs.durationMs += duration;
  state.jobs.maxDurationMs = Math.max(state.jobs.maxDurationMs, duration);
  const key = String(type);
  const byType = state.jobs.byType[key] || { total: 0, succeeded: 0, failed: 0 };
  byType.total += 1;
  if (status === 'succeeded') { state.jobs.succeeded += 1; byType.succeeded += 1; }
  else if (status === 'failed') {
    state.jobs.failed += 1;
    byType.failed += 1;
    pushRecent(state.jobs.recentFailed, { at: new Date().toISOString(), type: key, durationMs: round(duration) });
  }
  state.jobs.byType[key] = byType;
}
...
function snapshot() {
  ...
  return {
    ...
    jobs: {
      ...
      succeeded: state.jobs.succeeded,
      failed: state.jobs.failed,
      averageDurationMs: average(state.jobs.durationMs, state.jobs.total),
      maxDurationMs: round(state.jobs.maxDurationMs),
      byType: { ...state.jobs.byType },
      recentFailed: [...state.jobs.recentFailed],
    },
  };
}

function resetForTests() {
  for (const group of [state.requests, state.database, state.jobs]) {
...
module.exports = { beginRequest, finishRequest, recordQuery, recordJob, snapshot, resetForTests };
