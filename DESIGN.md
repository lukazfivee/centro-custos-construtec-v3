...
**Listas mobile (extensão desta sessão):** tabelas largas (`fornecedores`, `categorias`, `histórico`, `usuários`, `lançamentos`) recebem a classe `cc-mobile-table` no `<table>`; dentro de `@media (max-width: 800px)`, cada `<tr>` vira um cartão (`border-radius: 10px`, borda `var(--line)`, fundo `var(--white)`) e cada `<td>` com `data-label` mostra um rótulo maiúsculo à esquerda e o valor à direita — o mesmo princípio de "tabela → cartão" usado no Construtec Orçamentos para a tela de itens da proposta. `centros-custo` e `recorrentes` já usavam cartões nativos (`.center-card`) e não precisaram de conversão. Tabelas de orçamento/medições/contratos (módulo `budget-view`) ainda não foram convertidas — ver `HANDOFF-MOBILE-PARIDADE-DUPLA.md`.
