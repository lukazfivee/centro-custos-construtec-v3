...
(() => {
  ...
  window.ccRegisterList({
    ...
    render: (items) => items.map((item) => `<article class="center-card" data-center-id="${item.id}">
      <div class="center-card-head"><div class="center-card-icon">◫</div><span class="pill center-status-pill ${item.ativo ? 'ativo' : 'inativo'}">${item.ativo ? 'Ativo' : 'Inativo'}</span></div>
